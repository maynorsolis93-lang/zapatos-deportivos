...
const API_BASE_URL = window.location.hostname === 'localhost' 
  ...
  : 'https://kiro-shoes-backend.vercel.app/api'; // Ajustar cuando se despliegue el backend
...
/**
 * Carga la data del catalogo desde la API.
 * Si falla, intenta cargar desde JSON como fallback.
 */
async function loadStoreData() {
  try {
    ...
    state.heroSlides = Array.isArray(heroData.heroSlides) ? heroData.heroSlides : [];
    state.apiError = false;

    console.log('✅ Datos cargados desde API:', {
      productos: state.products.length,
      slides: state.heroSlides.length
    });

  } catch (error) {
    console.warn('⚠️ Error cargando desde API, intentando fallback a JSON:', error);
    
    // Fallback: intentar cargar desde JSON estático
    try {
      const response = await fetch('/data/store.json');
      if (!response.ok) {
        throw new Error(`No se pudo cargar store.json (${response.status})`);
      }
      ...
      console.log('✅ Datos cargados desde JSON (fallback):', {
        productos: state.products.length,
        slides: state.heroSlides.length
      });

    } catch (fallbackError) {
      console.error('❌ Error cargando datos (API y JSON fallaron):', fallbackError);
      state.heroSlides = [];
      state.products = [];
      state.apiError = true;
    }
